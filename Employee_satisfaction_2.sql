...
select count(e.employee_id) as total_count,l.city from employees e 
join departments d on e.department_id=d.department_id join locations l on d.location_id=l.location_id
group by l.city order by count(e.employee_id) desc limit 5;


with three_joi(emp_id,full_name,salary,dept_name,city) as
	(select e.employee_id,concat(e.first_name,' ',e.last_name) as full_name,e.salary,d.department_name,
    l.city from employees e join departments d on e.department_id=d.department_id join locations l on d.location_id=l.location_id),
	gr_joi(count_a,city) as
		(select count(emp_id),city from three_joi group by city order by count(emp_id) desc limit 5)
select t.* from three_joi t join gr_joi g on t.city=g.city order by t.emp_id;        
...
with three_joi(emp_id,full_name,salary,dept_name,city) as
	...
	gr_joi(count_a,city) as
		(select count(emp_id),city from three_joi group by city order by count(emp_id) desc limit 5)
select g.count_a,g.city from gr_joi as g;

with emp_det(emp_id,full_name,manager_id) as 
	(select e.employee_id,concat(e.first_name,' ',e.last_name),e.manager_id 
    from employees e where e.employee_id not in (select j.employee_id from job_history j) 
	and (year(current_date())-year(e.hire_date))>=10)
select n.emp_id as manager_id,n.full_name as manager_name,b.emp_id as employee_id,b.full_name as employee_name 
from emp_det n join emp_det b on n.emp_id=b.manager_id;
...
with three_joi(emp_id,full_name,salary,dept_name,city) as
	(select e.employee_id,concat(e.first_name,' ',e.last_name) as full_name,e.salary,d.department_name,
    l.city from employees e join departments d on e.department_id=d.department_id join locations l on d.location_id=l.location_id)
select * from three_joi;   

select b.hire_date,c.country_name,b.employee_id from 
(select * from employees e where e.employee_id not in (select j.employee_id from job_history j)) as b join departments d
on d.department_id=b.department_id join locations l on d.location_id=l.location_id join countries c on c.country_id=l.country_id; 

select b.*,d.*,l.* from (select * from employees e where e.employee_id not in (select j.employee_id from job_history j)) as b join departments d
on d.department_id=b.department_id join locations l on d.location_id=l.location_id;
...
(select * from employees e where e.employee_id not in (select j.employee_id from job_history j)) as b join departments d
on d.department_id=b.department_id join locations l on d.location_id=l.location_id join countries c 
on c.country_id=l.country_id group by c.country_name having avg(year(curdate())-year(b.hire_date))>=25; 


select c.country_name,count(b.employee_id) from 
(select * from employees e where e.employee_id not in (select j.employee_id from job_history j)) as b join departments d
on d.department_id=b.department_id join locations l on d.location_id=l.location_id join countries c 
on c.country_id=l.country_id group by c.country_name order by count(b.employee_id) desc; 
